...
async function main() {
  ...
  // common case: a single branch update whose commits form a simple range
  const single = branchUpdates.length === 1 ? branchUpdates[0] : undefined;
  if (
    single &&
    single.remoteSha !== ZERO_SHA &&
    commitExists(single.remoteSha) &&
    sameCommits(commits, revList([single.localSha, `^${single.remoteSha}`]))
  ) {
    const { status } = commitlint(["--from", single.remoteSha, "--to", single.localSha], {
      encoding: "utf8",
      stdio: "inherit",
    });
    return status ?? 1;
